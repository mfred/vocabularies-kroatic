...
class _VerifyEmailScreenState extends ConsumerState<VerifyEmailScreen> {
  ...
  Future<void> _checkVerified({bool silent = false}) async {
    ...
    try {
      ...
      if (verified) {
        ...
        // automatisch weiter. Hier zusätzlich Success-State zeigen, damit
        // der pushte VerifyEmailScreen nicht weiter den Warte-Button anbietet.
        _pollTimer?.cancel();
      ...
      body: SafeArea(
        child: TabletConstrained(
          child: SingleChildScrollView(
            ...
            child: _verified ? _buildSuccess(theme) : _buildWaiting(theme, scheme),
          ),
        ),
      ),
    );
  ...
  Widget _buildWaiting(ThemeData theme, ColorScheme scheme) {
    return Column(
  ...
  Widget _buildSuccess(ThemeData theme) {
    ...
    return Column(
      ...
      children: [
        const SizedBox(height: 24),
        const Icon(Icons.mark_email_read, size: 72, color: successGreen),
        const SizedBox(height: 16),
        Text(
          'Die Email-Adresse wurde bestätigt.',
          textAlign: TextAlign.center,
          style: theme.textTheme.titleLarge?.copyWith(
            fontWeight: FontWeight.w600,
            color: successGreen,
        ...
        Text(
          'Dein Account ist jetzt freigeschaltet.',
          textAlign: TextAlign.center,
          style: theme.textTheme.bodyMedium?.copyWith(
